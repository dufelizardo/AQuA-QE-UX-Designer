...
> **Nota de fase**: este documento descreve as skills **planejadas** para a Fase 1, na ordem do `agent_manifest.yaml` — nenhuma foi implementada ainda (repositório recém-criado, só a spec formal existe). Quando implementadas, viverão em `../../src/aqua_qe_ux_designer/skills/`, no formato definido em `../standards/skill_standard.md`. Tipos de entrada/saída referem-se às estruturas planejadas em `output_schema.md` (futuramente `../../src/aqua_qe_ux_designer/models/`).
>
