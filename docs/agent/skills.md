...
- **Descrição**: sintetiza e prioriza as 3 a 5 questões mais críticas combinando as recomendações de acessibilidade (`review_accessibility`) e as observações de usabilidade (`review_ux_specification`/`validate_ux_specification`) já existentes — nunca inventa um item novo que não esteja em uma das duas listas de entrada, só reordena/resume o que já existe.
- **Entrada**: `accessibility_recommendations: list[str]`, `review_notes: list[str]`.
- **Saída**: `list[str]`.
- **Efeitos colaterais**: chamada ao LLM gerador.
- **Erros esperados**: resposta do LLM não é JSON válido.
- **Dependências**: roda em `finalize_ux_specification`, depois de `review_notes` estar definido (por `validate_ux_specification` ou `review_ux_specification`).
